...
defineFeature(feature, test => {
  ...
  test('When user hasn’t searched for a city, show upcoming events from all cities.', ({ given, when, then }) => {
    ...
    then('the user should see the list of all upcoming events.', async () => {
      ...
      await waitFor(() => {
        ...
        expect(EventListItems.length).toBe(6); // Adjust this to match your mock data if needed
      });
    });
  });
  ...
  test('User should see a list of suggestions when they search for a city.', ({ given, when, then }) => {
    ...
    then('the user should receive a list of cities (suggestions) that match what they’ve typed', async () => {
      await waitFor(() => {
        ...
        expect(suggestionListItems).toHaveLength(2); // Adjust based on the mock data
      });
    });
  });
  ...
  test('User can select a city from the suggested list.', ({ given, and, when, then }) => {
    ...
    when(/^the user selects a city \(e.g., "(.*)"\) from the list$/, async (city) => {
      const user = userEvent.setup();
      await user.click(suggestionListItems[0]); // Simulate selecting the first city
    });

    then(/^their city should be changed to that city \(i.e., "(.*)"\)$/, (expectedCity) => {
      expect(citySearchInput.value).toBe(expectedCity);
    });
    ...
    and('the user should receive a list of upcoming events in that city', async () => {
      ...
      await waitFor(async () => {
        ...
        const filteredEvents = allEvents.filter(event => event.location === citySearchInput.value);
